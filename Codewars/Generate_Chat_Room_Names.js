function generateChatRoomNames(users) {
    ...
    for(let i = 0 ; i < lName.length ; i++){
        ...
        if (fil_fName.length == 1 ) {
            result.push(fName[i]) 
        }else if(fil_fName.length >= 1 && fil_lName.length >= 1 && fil_lName[i] === fil_lName[i+1]&& fil_fName.length != fil_lName.length){
            result.push(fName[i] +" "+ lName[i])
        }else if(fil_fName.length >= 1 && fil_lName.length >= 1 && fil_fName.length != fil_lName.length){
